...
class RandomPatch(object):
    ...
    There is a patch pool that stores randomly extracted patches from person images.
...
def build_transforms(
    ...
    **kwargs
    ...
    Args:
        height (int): target image height.
        width (int): target image width.
        transform (str or list of str, optional): transformations applied to model training.
            Default is 'random_flip'.
        norm_mean (list, optional): normalization mean values. Default is ImageNet means.
        norm_std (list, optional): normalization standard deviation values. Default is
    ...
    if not isinstance(transform, list):
        raise ValueError('transforms must be a list of strings, but found to be {}'.format(type(transform)))

    if len(transform) > 0:
        transform = [t.lower() for t in transform]
    ...
    train_transforms = []
    train_transforms += [Resize((height, width))]

    if 'random_flip' in transform:
        train_transforms += [RandomHorizontalFlip()]

    if 'random_crop' in transform:
        train_transforms += [Random2DTranslation(height, width)]

    if 'random_patch' in transform:
        train_transforms += [RandomPatch()]

    if 'color_jitter' in transform:
        train_transforms += [ColorJitter(brightness=0.2, contrast=0.15, saturation=0, hue=0)]

    train_transforms += [ToTensor()]
    train_transforms += [normalize]

    if 'random_erase' in transform:
        train_transforms += [RandomErasing(mean=norm_mean)]
